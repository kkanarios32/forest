...
print(
    f"""📚 {bib_file.relative_to(project_root)} -> {
...
for i, reference in enumerate(references):
    ...
    print(
        f"""  {csljson_file_i.relative_to(project_root)} -> {
    ...
    if os.path.exists(tree_file_i):
        ...
        with open(tree_file_i, "r", encoding="utf-8") as f:
            ...
            bib_filenames_i = [
                filename for filename in first_line_json if filename != bib_filename
            ]
    ...
    if tree_file_i.exists():
        with open(tree_file_i, "r", encoding="utf-8") as f:
            ...
            if len(existing_bib_filenames_i) >= len(bib_filenames_i) or set(
                existing_bib_filenames_i
            ) == set(bib_filenames_i):
